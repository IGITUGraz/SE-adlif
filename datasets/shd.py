...
class SHDYin(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        train_X = np.load("/scratch/baronig/data_sets/SHD/trainX_4ms.npy")
        train_y = np.load("/scratch/baronig/data_sets/SHD/trainY_4ms.npy").astype(int)
        test_X = np.load("/scratch/baronig/data_sets/SHD/testX_4ms.npy")
        test_y = np.load("/scratch/baronig/data_sets/SHD/testY_4ms.npy").astype(int)
        tensor_trainX = torch.tensor(
            train_X, dtype=torch.float32
        )  # transform to torch tensor
        tensor_trainY = torch.tensor(train_y, dtype=torch.int64)
        train_dataset = data.TensorDataset(tensor_trainX, tensor_trainY)
        self.train_loader = data.DataLoader(
            train_dataset, batch_size=self.batch_size, shuffle=True
        )
        tensor_testX = torch.tensor(
            test_X, dtype=torch.float32
        )  # transform to torch tensor
        tensor_testY = torch.tensor(test_y, dtype=torch.int64)
